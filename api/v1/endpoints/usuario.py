...
from schemas.usuario_schema import UsuarioSchemaBase, UsuarioSchemaCreate, UsuarioSchemasArtigos, UsuarioSchemaUpdate
...
@router.get('/artigos', response_model=List[UsuarioSchemasArtigos], status_code=status.HTTP_200_OK)
async def get_usuarios_artigos(db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(UsuarioModel)
        result = await session.execute(query)
        usuarios = result.scalars().unique().all()
...
async def get_usuario(usuario_id: int, usuario: UsuarioSchemaUpdate, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(UsuarioModel).filter(UsuarioModel.id == usuario_id)
        result = await session.execute(query)
        ...
        if usuario_up:
            ...
            raise HTTPException(detail='Campos estão em branco', status_code=status.HTTP_404_NOT_FOUND)
            
            
@router.delete('/{usuario_id}', response_model=UsuarioSchemasArtigos, status_code=status.HTTP_200_OK)
async def get_usuario(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(UsuarioModel).filter(UsuarioModel.id == usuario_id)
        result = await session.execute(query)
